...
def pokemon_search():
    ...
    try:
        if request.method == 'POST':
            ...
            found_pokemons = search_pokemons_by_name(search_pokemon_name=pokemon_name, pokedex=pokemons_results)
...
def not_found(error=None):
    ...
    return Response(response=f'Resource not found: "{request.url}"', status=404, mimetype='application/json')
